...
base1 <-select(tvivienda,c(vid,P5_2))

# Recode to NA para los montos que no responde o no sabe 
base1$P5_2[base1$P5_2 == 999999888] <- NA
base1$P5_2[base1$P5_2 == 999999999] <- NA
...
#gastos_creditos_mes <- gastos_creditos_mes %>% 
#  left_join(select(base1,P5_2,P5_1,vid), by = "vid") %>% 
#  filter(P5_1 == 4)
...
ingreso_gasto = full_join(ing_viv, base1, by = "vid")
...
ingreso_gasto = ingreso_gasto %>%
  rowwise() %>%
  mutate(prop_gasto = P5_2 / ingreso_viv)
...
ingreso_gasto$indice_aseq = ifelse(ingreso_gasto$prop_gasto > 0.30 ,
                          1,
                          ifelse(ingreso_gasto$prop_gasto <= 0.30, 2,
                                   ingreso_gasto$prop_gasto
                                 ))
...
#write.csv(ingreso_gasto,"KPI/indice_asequibilidad.csv")
